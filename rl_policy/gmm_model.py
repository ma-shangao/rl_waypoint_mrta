...
class GaussianMixture(torch.nn.Module):
    ...
    def _init_params(self):
        ...
        # (1, k, 1)
        self.pi = torch.nn.Parameter(torch.Tensor(1, self.n_components, 1), requires_grad=True)
        torch.nn.init.uniform_(self.pi, 1. / self.n_components, 1. / self.n_components + 0.01)
    ...
    def calculate_matmul_n_times(self, n_components, mat_a, mat_b):
        """
    ...
    def calculate_matmul(self, mat_a, mat_b):
        """
        Calculate matrix product of two matrics with mat_a[0] >= mat_b[0].
        Bypasses torch.matmul to reduce memory footprint.
        args:
    ...
    def forward(self, x):
        ...
        pi = torch.squeeze(torch.max(weighted_log_prob, 1)[1].type(torch.LongTensor))
        weighted_log_prob_sum = weighted_log_prob.gather(0, pi[:, None, None])
        return pi, torch.squeeze(p_k / (p_k.sum(1, keepdim=True))), weighted_log_prob_sum
        ### pi, logits, log_p
